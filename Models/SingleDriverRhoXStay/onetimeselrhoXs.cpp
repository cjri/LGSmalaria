...
int main(int argc, char *argv[]){
	...
	double rho1_final=0;
	double rho2_final=0;
	double rx_final=0;  //Point of change in recombination rate
	double xf_final=0;
	...
	for (int r=1; r<=reps; r++) {
		...
		od.rho1 = 1e-6;
		od.rho2 = 1e-6;
		od.rx = floor(gsl_rng_uniform(rgen)*p);
		...
		for (int it=0;it<1000000;it++){
			...
			gsl_vector_set(ss,2,od.rho1/4);
			gsl_vector_set(ss,3,od.rho2/4);
			gsl_vector_set(ss,4,100);
			...
			if (last_fit>best_fit) {
				...
				rx_final=gsl_vector_get(s->x,4);
				xf_final=gsl_vector_get(s->x,5);

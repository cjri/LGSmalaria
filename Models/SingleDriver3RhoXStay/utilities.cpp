...
void SetXXVector(opt_data od, gsl_vector* xx) {
	...
	gsl_vector_set(xx,5,od.rho4);	//Recombination rate driver right
	gsl_vector_set(xx,6,od.rx1);	//Recombination rate switch point
	gsl_vector_set(xx,7,od.rx2);	//Recombination rate switch point
	gsl_vector_set(xx,8,od.rx3);	//Recombination rate switch point
	gsl_vector_set(xx,9,od.xf);	//Driver frequency after selection
}
...
int CheckParams(const gsl_vector *xx, double *p) {
	...
	double rho4=gsl_vector_get(xx,5);
	double rx1=gsl_vector_get(xx,6);
	double rx2=gsl_vector_get(xx,7);
	double rx3=gsl_vector_get(xx,8);
	double xf=gsl_vector_get(xx,9);
	if ((rho1<0)||(rho2<0)||(rho3<0)||(rho4<0)) {
		chk=0;
	}
	if ((rx1>=rx2)||(rx1>=rx3)||(rx2>=rx3)) {
		chk=0;
	}
	if ((rx1<0 || rx1>p[0])||(rx2<0 || rx2>p[0])||(rx3<0 || rx3>p[0])) {
		chk=0;
	}
	if (x<=0.5 || x>=1) {
		chk=0;
...
void FindFrequencies (int driver, vector<rec> traj, vector<rec>& traj_inf, const gsl_vector *xx) {
	...
	for (unsigned int j=0;j<traj.size();j++) {
		...
		if (j!=i) {
			...
			double Delta_ix3=abs(traj_inf[i].pos-traj[rx3].pos);
			double Delta_jx3=abs(traj_inf[j].pos-traj[rx3].pos);
			double Delta_x1x2=abs(traj[rx2].pos-traj[rx1].pos);
			double Delta_x2x3=abs(traj[rx3].pos-traj[rx2].pos);
			...
			if (i<=rx1) {
				...
				if (j>rx3) {
					rd=(rho4*Delta_jx3)+(rho3*Delta_x2x3)+(rho2*Delta_x1x2)+(rho1*Delta_ix1);
				}
			...
			if ((i>rx1)&&(i<=rx2)) {
				...
				if ((j>rx2)&&(j<=rx3)) {
					rd=(rho2*Delta_ix2)+(rho3*Delta_jx2);
				}
				if (j>rx3) {
					rd=(rho2*Delta_ix2)+(rho3*Delta_x2x3)+(rho4*Delta_jx3);
				}
			}
			...
			if ((i>rx2)&&(i<=rx3)) {
				...
				if (j>rx3) {
					rd=(rho4*Delta_jx3)+(rho3*Delta_ix3);
				}
			}
			
			if (i>rx3) {
				if (j<=rx1) {
					rd=(rho1*Delta_jx1)+(rho2*Delta_x1x2)+(rho3*Delta_x2x3)+(rho4*Delta_ix3);
				}
				
				if ((j>rx1)&&(j<=rx2)) {
					rd=(rho2*Delta_jx2)+(rho3*Delta_x2x3)+(rho4*Delta_ix3);
				}
				
				if ((j>rx2)&&(j<=rx3)) {
					rd=(rho3*Delta_jx3)+(rho4*Delta_ix3);
				}
				if (j>rx3) {
					rd=rho4*Delta_ij;
				}

...
int main(int argc, char *argv[]){
	...
	for (int r=1; r<=reps; r++) {
		...
		od.rho2 = 1e-6;
		od.rho3 = 1e-6;
		od.rx1 = floor((0.49*gsl_rng_uniform(rgen)*p));
		od.rx2 = floor((0.5*p)+(0.49*gsl_rng_uniform(rgen)*p));
		...
		for (int it=0;it<1000000;it++){
			...
			gsl_vector_set(ss,2,od.rho1);
			gsl_vector_set(ss,3,od.rho2);
			gsl_vector_set(ss,4,od.rho3);
			gsl_vector_set(ss,5,100);
			gsl_vector_set(ss,6,100);
			gsl_vector_set(ss,7,0.1);
			//cout << "Done set ss\n";
			...
			if (last_fit>best_fit) {
				...
				rx1_final=gsl_vector_get(s->x,5);
				rx2_final=gsl_vector_get(s->x,6);
				xf_final=gsl_vector_get(s->x,7);
